...
export default function SpendingTrendChart({
  ...
  return (
    <AreaChart width={width} height={height} data={data} margin={{ top: 8, right: 2, left: -8, bottom: 0 }} tabIndex={-1} accessibilityLayer={false}>
      ...
      <Tooltip cursor={false} formatter={(value) => [formatValue(Number(value)), "Spent"]} labelStyle={{ color: "#F1F5F9", fontWeight: 700 }} contentStyle={{ border: 0, borderRadius: 14, background: "#1C1F35", boxShadow: "0 12px 28px rgba(0,0,0,0.5)" }} wrapperStyle={{ outline: "none", border: 0 }} />
