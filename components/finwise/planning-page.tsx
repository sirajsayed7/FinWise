  ...
  return <section>
    ...
    <div className="grid gap-4">
      <section className="fin-card">
        <div className="flex items-center gap-3"><span className="fin-icon bg-violet-50 text-violet-600"><PiggyBank size={22} /></span><div><h2 className="fin-title">Monthly budgets</h2><p className="fin-copy">Get alerted before a category crosses its limit.</p></div></div>
        ...
        <div className="mt-4 grid gap-2">{budgets.length ? budgets.map((budget) => { const spent = metrics?.categoryTotals.filter((row) => row.month === metrics.latestDate?.slice(0, 7) && row.category === budget.category).reduce((sum, row) => sum + row.amount, 0) ?? 0; const progress = Math.min(100, budget.amount ? spent / budget.amount * 100 : 0); return <article key={`${budget.category}-${budget.period}`} className="rounded-[17px] bg-slate-50 p-3 ring-1 ring-slate-200"><div className="flex items-center justify-between gap-3"><div><p className="text-[14px] font-extrabold">{budget.category}</p><p className="text-[12px] font-semibold text-slate-500">QAR {spent.toFixed(2)} of QAR {budget.amount.toFixed(2)}</p></div><button aria-label={`Delete ${budget.category} budget`} onClick={() => onDeleteBudget(budget)} className="fin-icon text-red-500"><Trash2 size={18} /></button></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200"><div className={progress >= 100 ? "h-full bg-red-500" : progress >= 80 ? "h-full bg-amber-500" : "h-full bg-violet-600"} style={{ width: `${progress}%` }} /></div></article>; }) : <Empty text="No budgets yet." />}</div>
      </section>
      <section className="fin-card">
        <div className="flex items-center gap-3"><span className="fin-icon bg-emerald-50 text-emerald-600"><Target size={22} /></span><div><h2 className="fin-title">Financial goals</h2><p className="fin-copy">Track savings targets with clear progress.</p></div></div>
        ...
        <div className="mt-4 grid gap-2">{goals.length ? goals.map((goal) => { const progress = Math.min(100, goal.currentAmount / goal.targetAmount * 100); return <article key={goal.id} className="rounded-[17px] bg-emerald-50/60 p-3 ring-1 ring-emerald-100"><div className="flex items-start justify-between"><div><p className="text-[14px] font-extrabold">{goal.name}</p><p className="text-[12px] font-semibold text-slate-500">{progress.toFixed(0)}% ? QAR {goal.currentAmount.toFixed(2)} / {goal.targetAmount.toFixed(2)}</p></div><button aria-label={`Delete ${goal.name}`} onClick={() => onDeleteGoal(goal.id)} className="fin-icon text-red-500"><Trash2 size={18} /></button></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-emerald-100"><div className="h-full bg-emerald-500" style={{ width: `${progress}%` }} /></div></article>; }) : <Empty text="No goals yet." />}</div>
      </section>
